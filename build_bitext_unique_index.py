...
while len(hits):
	for bitext_element in hits:
		...
		bitext_unique_key = bitext_element["_source"]["commentId"]
		response_bitext_unique = json.loads(requests.get("http://localhost:9200/bitext_unique/_all/" + bitext_unique_key).text)
		if "found" in response_bitext_unique and response_bitext_unique["found"]:
			previous_count = response_bitext_unique["_source"]["count"]
			previous_average_score = response_bitext_unique["_source"]["averageScore"]
			previous_categories = response_bitext_unique["_source"]["category"]
			separator = ", "
		update_query = {
			"doc":{
		...
		bitext_unique_response = requests.post("http://localhost:9200/bitext_unique/" + bitext_element["_source"]["hotelSequence"] + "/" + bitext_unique_key + "/_update", data = json.dumps(update_query))
